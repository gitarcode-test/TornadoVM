...
public class TestReductionsIntegers extends TornadoTestBase {
  ...
  /**
   * First approach: use annotations in the user code to identify the reduction variables. This is a
   * similar approach to OpenMP and OpenACC.
   *
   * @param input
   * @param result
   */
  ...
  private static void maxReductionAnnotation2(
      IntArray input, @Reduce IntArray result, int neutral) {
    result.set(0, neutral);
    for (@Parallel int i = 0; i < input.getSize(); i++) {
  ...
  public void testSequentialReduction() throws TornadoExecutionPlanException {
    ...
    Random r = new Random();
    IntStream.range(0, SMALL_SIZE * 2)
        .parallel()
        .forEach(
            i -> {
    ...
    assertThat((double) sequential.get(0), closeTo(result.get(0), 0.001f));
  }
  ...
  public void testMinReduction2() throws TornadoExecutionPlanException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0") //
            ...
            .task(
                "t0",
                TestReductionsIntegers::minReductionAnnotation2,
                input,
                result,
                Integer.MAX_VALUE) //
            .transferToHost(DataTransferMode.EVERY_EXECUTION, result);
