...
/**
 * Tests TornadoVM compilation under different scenarios, when not performing inlining in the method
 * passed to the task.
 *
 * <p>How to run? <code>
...
public class TestMultipleFunctions extends TornadoTestBase {
  ...
  /**
   * Test to check we can generate vector types for the method signature and non-main kernel
   * functions.
   */
  ...
  public void test02() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < c.getSize(); i++) {
      assertThat((a.get(i) + (a.get(i) + b.get(i))), equalTo(c.get(i)));
    }
  }
  ...
  public void test03() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < c.getSize(); i++) {
      assertThat((a.get(i) + (a.get(i) + (a.get(i) + b.get(i)))), equalTo(c.get(i)));
    }
  }
  ...
  public void test04() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < c.getSize(); i++) {
      assertThat((a.get(i) + a.get(i)) + (b.get(i) * b.get(i)), equalTo(c.get(i)));
    }
  }
  ...
  public void test05() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < c.getSize(); i++) {
      assertThat((double) checker.get(i), closeTo(c.get(i), 0.01f));
    }
  }
  ...
  public void testVector01() throws TornadoExecutionPlanException {
    ...
    assertThat(result.getX(), equalTo(c.getX()));
    assertThat(result.getY(), equalTo(c.getY()));
    assertThat(result.getW(), equalTo(c.getW()));
    assertThat(result.getZ(), equalTo(c.getZ()));
  }

  /**
   * Tests {@link uk.ac.manchester.tornado.api.common.Access} pattern when calling a method and
   * writing to one of the parameters in the callee.
   */
  public void caller1(
      IntArray calleeRead,
      int ignoreParam1,
      IntArray callerReadCalleeWrite,
      int ignoreParam2,
      IntArray callerRead,
      IntArray callerWrite) {
  ...
  public void caller3(
      IntArray callerReadWrite,
      IntArray callee1Read,
      IntArray callee1Write,
      IntArray callee2ReadWrite,
      IntArray callee2Read) {
  ...
  public void testSingleTask() throws TornadoExecutionPlanException {
    ...
    caller1(
        testArrays.calleeReadSeq,
        testArrays.ignoreParam1,
        testArrays.callerReadCalleeWriteSeq,
        testArrays.ignoreParam2,
        testArrays.callerReadSeq,
        testArrays.callerWriteSeq);
    ...
    for (int i = 0; i < testArrays.calleeReadTor.getSize(); i++) {
      assertThat(testArrays.calleeReadSeq.get(i), equalTo(testArrays.calleeReadTor.get(i)));
    }
    for (int i = 0; i < testArrays.callerReadCalleeWriteSeq.getSize(); i++) {
      assertThat(testArrays.callerReadCalleeWriteSeq.get(i), equalTo(testArrays.callerReadCalleeWriteTor.get(i)));
    }
    for (int i = 0; i < testArrays.callerReadSeq.getSize(); i++) {
      assertThat(testArrays.callerReadSeq.get(i), equalTo(testArrays.callerReadTor.get(i)));
    }
    for (int i = 0; i < testArrays.callerWriteSeq.getSize(); i++) {
      assertThat(testArrays.callerWriteSeq.get(i), equalTo(testArrays.callerWriteSeq.get(i)));
    }
  }

  /**
   * Tests {@link uk.ac.manchester.tornado.api.common.Access} pattern when calling two methods from
   * different tasks, passing the same parameter to both tasks, and writing in only one callee.
   */
  @Test
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0") //
            ...
            .task(
                ...
                testArrays.callerWriteTor) //
            .task(
                "t1", testTaskAccesses::caller2, testArrays.callerReadTor, testArrays.calleeReadTor)
            .transferToHost(
                DataTransferMode.EVERY_EXECUTION,
                testArrays.callerReadCalleeWriteTor,
                testArrays.callerWriteTor,
  ...
  /**
   * Tests {@link uk.ac.manchester.tornado.api.common.Access} pattern when calling three methods
   * from different tasks. Performs a combination of {@link #testMultipleTasks} and {@link
   * #testSingleTask}.
   */
  @Test
  public void testMultipleTasksMultipleCallees() {
    ...
    caller1(
        arrays.calleeReadSeq,
        arrays.ignoreParam1,
        arrays.callerReadCalleeWriteSeq,
        arrays.ignoreParam2,
        arrays.callerReadSeq,
        arrays.callerWriteSeq);
    caller2(arrays.callerReadSeq, arrays.calleeReadSeq);
    caller3(
        arrays.callerReadWriteSeq,
        arrays.calleeReadSeq,
        arrays.callee1WriteSeq,
        arrays.callerReadCalleeWriteSeq,
        arrays.callee2ReadSeq);
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0")
            ...
            .task(
                "t1",
                testTaskAccesses::caller2,
                arrays.callerReadTor, //
                arrays.calleeReadTor) //
            .task(
                "t2",
                testTaskAccesses::caller3, //
    ...
    for (int i = 0; i < arrays.calleeReadTor.getSize(); i++) {
      assertThat(arrays.calleeReadSeq.get(i), equalTo(arrays.calleeReadTor.get(i)));
    }
    for (int i = 0; i < arrays.callerReadCalleeWriteSeq.getSize(); i++) {
      assertThat(arrays.callerReadCalleeWriteSeq.get(i), equalTo(arrays.callerReadCalleeWriteTor.get(i)));
    }
    for (int i = 0; i < arrays.callerReadSeq.getSize(); i++) {
      assertThat(arrays.callerReadSeq.get(i), equalTo(arrays.callerReadTor.get(i)));
    }
    for (int i = 0; i < arrays.callerWriteSeq.getSize(); i++) {
      assertThat(arrays.callerWriteSeq.get(i), equalTo(arrays.callerWriteSeq.get(i)));
    }
    for (int i = 0; i < arrays.callerReadWriteSeq.getSize(); i++) {
      assertThat(arrays.callerReadWriteSeq.get(i), equalTo(arrays.callerReadWriteTor.get(i)));
    }
    for (int i = 0; i < arrays.callee1WriteSeq.getSize(); i++) {
      assertThat(arrays.callee1WriteSeq.get(i), equalTo(arrays.callee1WriteTor.get(i)));
    }
    for (int i = 0; i < arrays.callee2ReadSeq.getSize(); i++) {
      assertThat(arrays.callee2ReadSeq.get(i), equalTo(arrays.callee2ReadTor.get(i)));
    }
  }
  ...
  // CHECKSTYLE:OFF
  /**
   * Tests if methods/functions invoked from different places in the call graph do not get compiled
   * twice. A → B → D ↘ C ↗ If compiled twice, it will generate a runtime exception when launching
   * the kernel.
   */
  // CHECKSTYLE:ON
  // @formatter:on
  ...
  public void testNoDoubleCompilation() throws TornadoExecutionPlanException {
    ...
    assertThat(-1, equalTo(arr.get(0)));
  }
