...
public class TestAtomics extends TornadoTestBase {
   ...
   *
   * @param a Input array. It stores the addition with an atomic variable.
   */
  ...
  /**
   * Approach using an API for Atomics. This provides atomics using the Java semantics (block a
   * single elements). Note that, in OpenCL, this single elements has to be present in the device's
   * global memory.
   *
  ...
  /**
   * This example combines an atomic created inside the compute kernel with an atomic passed as an
   * argument.
   *
   * @param input Input array
   * @param ai Atomic Integer stored in Global Memory (atomic-region)
   */
  ...
  public void testAtomic05_precompiled() throws TornadoExecutionPlanException {
    ...
    TornadoDevice defaultDevice =
        TornadoRuntimeProvider.getTornadoRuntime().getBackend(0).getDevice(deviceNumber);
