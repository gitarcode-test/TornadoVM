...
public class GraphicsTests extends TornadoTestBase {
  ...
  private static void testPhiNode(ImageFloat3 vertices, ImageFloat depths, Matrix4x4Float invK) {
    ...
    final Float3 vertex =
        (depth > 0) ? Float3.mult(rotate(invK, pix), depth) : new Float3(0f, 0f, 0f);
  ...
  private static Float3 rotate(Matrix4x4Float m, Float3 v) {
    return new Float3(
        Float3.dot(m.row(0).asFloat3(), v),
        Float3.dot(m.row(1).asFloat3(), v),
  ...
  public static void trackPose(
      final ImageFloat8 results,
      final ImageFloat3 verticies,
      final ImageFloat3 normals,
      final ImageFloat3 referenceVerticies,
      final ImageFloat3 referenceNormals,
      final Matrix4x4Float currentPose,
      final Matrix4x4Float view,
    ...
    for (@Parallel int y = 0; y < results.Y(); y++) {
      for (@Parallel int x = 0; x < results.X(); x++) {
        ...
        if (normals.get(x, y).getX() == Constants.INVALID) {
          ...
          final Float3 projectedVertex =
              GraphicsMath.rigidTransform(currentPose, verticies.get(x, y));

          // rotate + translate projected position
          final Float3 projectedPos = GraphicsMath.rigidTransform(view, projectedVertex);

          final Float2 projectedPixel =
              Float2.add(Float2.mult(projectedPos.asFloat2(), 1f / projectedPos.getZ()), 0.5f);

          boolean isNotInImage =
              (projectedPixel.getX() < 0)
          ...
          if (isNotInImage) {
            ...
            final Int2 refPixel =
                new Int2((int) projectedPixel.getX(), (int) projectedPixel.getY());
            ...
            if (referenceNormal.getX() == Constants.INVALID) {
              ...
              final Float3 diff =
                  Float3.sub(
                      referenceVerticies.get(refPixel.getX(), refPixel.getY()), projectedVertex);
              ...
              if (Float3.length(diff) > distanceThreshold) {
                ...
                if (Float3.dot(projectedNormal, referenceNormal) < normalThreshold) {
                  ...
                  final Float8 tracking =
                      new Float8(
                          referenceNormal.getX(),
                          referenceNormal.getY(),
                          referenceNormal.getZ(),
                          b.getX(),
                          b.getY(),
                          b.getZ(),
                          Float3.dot(referenceNormal, diff),
                          (float) Constants.GREY);
  ...
  public static void computeRigidTransform(
      Matrix4x4Float matrix, VectorFloat3 points, VectorFloat3 output) {
  ...
  public static void raycast(
    ...
    for (@Parallel int y = 0; y < verticies.Y(); y++) {
      for (@Parallel int x = 0; x < verticies.X(); x++) {
        final Float4 hit =
            GraphicsMath.raycastPoint(
                volume, volumeDims, x, y, view, nearPlane, farPlane, smallStep, largeStep);
  ...
  public static void testRayCastPointIsolation(
      ImageFloat4 output,
      ImageFloat3 verticies,
      VolumeShort2 volume,
      Float3 volumeDims,
    ...
    for (@Parallel int i = 0; i < verticies.X(); i++) {
      for (@Parallel int j = 0; j < verticies.Y(); j++) {
        final Float4 hit =
            GraphicsMath.raycastPoint(
                volume, volumeDims, i, j, view, nearPlane, farPlane, smallStep, largeStep);
        output.set(i, j, hit);
      }
    }
  }
  ...
  public static void integrate(
      ImageFloat filteredDepthImage,
      Matrix4x4Float invTrack,
      Matrix4x4Float K,
      Float3 volumeDims,
      VolumeShort2 volume,
      float mu,
    ...
    for (@Parallel int y = 0; y < volume.Y(); y++) {
      for (@Parallel int x = 0; x < volume.X(); x++) {
        ...
        for (int z = 0;
            z < volume.Z();
            z++, pos = add(pos, integrateDelta), cameraX = add(cameraX, cameraDelta)) {
          ...
          if (diff > -mu) {
            ...
            final Float2 data =
                mult(new Float2(inputValue.getX(), inputValue.getY()), constantValue1);

            final float dx =
                TornadoMath.clamp(
                    ((data.getY() * data.getX()) + sdf) / (data.getY() + 1f), -1f, 1f);
            final float dy = min(data.getY() + 1f, maxWeight);

            final Float2 floatValue = mult(new Float2(dx, dy), constantValue2);
            final Short2 outputValue =
                new Short2((short) floatValue.getX(), (short) floatValue.getY());
  ...
  public static void volumeOps(
      VectorFloat3 output, VolumeShort2 volume, final Float3 dim, final Float3 point) {
   ...
   *
   * @param k - camera parameters {f_x,f_y,x_0,y_0} where {f_x,f_y} specifies the focal length of
   *     the camera and {x_0,y_0} the principle point
  ...
  public void testMm2Meters() {
    ...
    for (int i = 0; i < destSize * destSize; i++) {
      assertThat((double) dest.get(i), closeTo(destSeq.get(i), 0.001));
    }
  }
  ...
  public void testBilateralFilter() {
    ...
    for (int i = 0; i < scaledSize * scaledSize; i++) {
      assertThat("index = " + i, (double) destSeq.get(i), closeTo(dest.get(i), 0.001));
    }
  }
  ...
  public void testRotate() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      assertThat((double) s.getS0(), closeTo(o.getS0(), 0.001));
      assertThat((double) s.getS1(), closeTo(o.getS1(), 0.001));
      assertThat((double) s.getS2(), closeTo(o.getS2(), 0.001));
    }
  }
  ...
  public void testTrackPose() {
    ...
    // Sequential execution
    trackPose(
        sequantialPyramidTrackingResults,
        pyramidVertices,
        pyramidNormals,
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        ...
        assertThat((double) s.getS1(), closeTo(o.getS1(), 0.001));
        assertThat((double) s.getS2(), closeTo(o.getS2(), 0.001));
        assertThat((double) s.getS3(), closeTo(o.getS3(), 0.001));
        assertThat((double) s.getS4(), closeTo(o.getS4(), 0.001));
        assertThat((double) s.getS5(), closeTo(o.getS5(), 0.001));
        assertThat((double) s.getS6(), closeTo(o.getS6(), 0.001));
        assertThat((double) s.getS7(), closeTo(o.getS7(), 0.001));
      }
    }
  }
  ...
  public void testPhiNode() {
    ...
    assertThat((double) s.getS0(), closeTo(o.getS0(), 0.01));
    assertThat((double) s.getS1(), closeTo(o.getS1(), 0.01));
    assertThat((double) s.getS2(), closeTo(o.getS2(), 0.01));
  }
      ...
      for (int j = 0; j < size; j++) {
        for (int k = 0; k < size; k++) {
          volume.set(
              ...
              j,
              k,
              new Short2((short) (r.nextInt() + i + j + k), (short) (r.nextInt() + i + j - k)));
        }
      }
    }

    GraphicsTests.raycast(
        verticiesSequential,
        normalsSequential,
        volume,
        volumeDims,
  ...
  public void testRaycastPoint() {
    ...
    GraphicsTests.testRayCastPointIsolation(
        outputSeq, verticies, volume, volumeDims, view, nearPlane, farPlane, largeStep, smallStep);

    TaskGraph taskGraph =
        new TaskGraph("s0") //
            .transferToDevice(
                DataTransferMode.FIRST_EXECUTION, verticies, volume, volumeDims, view) //
            .task(
                "t0",
                GraphicsTests::testRayCastPointIsolation,
                output,
    ...
    for (int i = 0; i < output.X(); i++) {
      for (int j = 0; j < output.Y(); j++) {
        ...
        assertThat((double) s.getX(), closeTo(o.getX(), 0.001));
        assertThat((double) s.getY(), closeTo(o.getY(), 0.001));
        assertThat((double) s.getZ(), closeTo(o.getZ(), 0.001));
        assertThat((double) s.getW(), closeTo(o.getW(), 0.001));
      }
    }
  }
  ...
  public void testIntegrate() {
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0") //
            .transferToDevice(
                DataTransferMode.FIRST_EXECUTION, filteredDepthImage, invTrack, m2, volumeDims) //
            .task(
                "t0",
                GraphicsTests::integrate,
                filteredDepthImage,
                invTrack,
                m2,
                volumeDims,
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        for (int k = 0; k < size; k++) {
          ...
          assertThat(expected.getX(), equalTo(got.getX()));
          assertThat(expected.getY(), equalTo(got.getY()));
        }
      }
    }
  }
  ...
  public void testRenderTrack() {
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        track.set(
            ...
            j,
            new Float8(
                random(r), random(r), random(r), random(r), random(r), random(r), random(r),
                random(r)));
      }
    }
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        ...
        assertThat(expected.getX(), equalTo(o.getX()));
        assertThat(expected.getY(), equalTo(o.getY()));
        assertThat(expected.getZ(), equalTo(o.getZ()));
      }
    }
  }
  ...
  public void testVolumeGrad() {
    ...
    for (int i = 0; i < output.getLength(); i++) {
      ...
      assertThat("difference on index " + i + " s0", (double) s.getS0(), closeTo(o.getS0(), 0.01f));
      assertThat("difference on index " + i + " s1", (double) s.getS1(), closeTo(o.getS1(), 0.01f));
      assertThat("difference on index " + i + " s2", (double) s.getS2(), closeTo(o.getS2(), 0.01f));
    }
  }
  ...
  public void testCameraMatrix() {
    ...
    for (int i = 0; i < m.getNumRows(); i++) {
      for (int j = 0; j < m.getNumColumns(); j++) {
        assertThat((double) seq.get(i, j), closeTo(m.get(i, j), 0.01f));
      }
    }
  }
  ...
  public void testRenderVolume() {
    ...
    VolumeShort2 volumeSeq =
        new VolumeShort2(volumeSize.getX(), volumeSize.getY(), volumeSize.getZ());
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0") //
            .transferToDevice(
                DataTransferMode.FIRST_EXECUTION, volume, volumeDims, scenePose, light, ambient) //
            .task(
                "t0",
                Renderer::renderVolume,
                output,
    ...
    for (int i = 0; i < output.X(); i++) {
      for (int j = 0; j < output.Y(); j++) {
        ...
        assertThat("index = " + i + ", " + j, s.getX(), equalTo(o.getX()));
        assertThat("index = " + i + ", " + j, s.getY(), equalTo(o.getY()));
        assertThat("index = " + i + ", " + j, s.getZ(), equalTo(o.getZ()));
        assertThat("index = " + i + ", " + j, s.getW(), equalTo(o.getW()));
      }
    }
  }
  ...
  private Float8 createFloat8() {
    ...
    Float8 float8 =
        new Float8(
            random(r), random(r), random(r), random(r), random(r), random(r), random(r), random(r));
    return float8;
  }
  ...
  public void testMapReduceSlam() {
    ...
    for (int i = 0; i < output.getSize(); i++) {
      assertThat((double) outputSeq.get(i), closeTo(output.get(i), 0.1f));
    }
  }
  ...
  public void testVS() {
    ...
    for (int i = 0; i < output.length; i++) {
      assertThat((double) seq[i], closeTo(output[i], 0.001f));
    }
  }
