...
public class TestTensorTypes extends TornadoTestBase {
  ...
  public static void tensorAdditionFloat16(
      TensorFP16 tensorA, TensorFP16 tensorB, TensorFP16 tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
      tensorC.set(i, HalfFloat.add(tensorA.get(i), tensorB.get(i)));
    }
  }

  public static void tensorAdditionFloat32(
      TensorFP32 tensorA, TensorFP32 tensorB, TensorFP32 tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
      tensorC.set(i, tensorA.get(i) + tensorB.get(i));
    }
  }

  public static void tensorAdditionFloat64(
      TensorFP64 tensorA, TensorFP64 tensorB, TensorFP64 tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
      tensorC.set(i, tensorA.get(i) + tensorB.get(i));
    }
  }

  public static void tensorAdditionInt16(
      TensorInt16 tensorA, TensorInt16 tensorB, TensorInt16 tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
      tensorC.set(i, (short) (tensorA.get(i) + tensorB.get(i)));
    }
  }

  public static void tensorAdditionInt32(
      TensorInt32 tensorA, TensorInt32 tensorB, TensorInt32 tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
      tensorC.set(i, tensorA.get(i) + tensorB.get(i));
    }
  }

  public static void tensorAdditionInt64(
      TensorInt64 tensorA, TensorInt64 tensorB, TensorInt64 tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
      tensorC.set(i, tensorA.get(i) + tensorB.get(i));
    }
  }

  public static void tensorAdditionByte(
      TensorByte tensorA, TensorByte tensorB, TensorByte tensorC) {
    for (@Parallel int i = 0; i < tensorC.getSize(); i++) {
  ...
  public void testHelloTensorAPI() {
    ...
    assertThat("Expected shape does not match", tensorA.getShape().toString(), equalTo("Shape{dimensions=[64, 64, 64]}"));
    assertThat("Expected data type does not match", tensorA.getDTypeAsString(), equalTo("HALF_FLOAT"));
    assertThat("Expected TensorFlow shape string does not match", tensorA.getShape().toTensorFlowShapeString(), equalTo("[64,64,64]"));
    assertThat("Expected ONNX shape string does not match", tensorA.getShape().toONNXShapeString(), equalTo("{dim_0: 64, dim_1: 64, dim_2: 64}"));
  }
  ...
  public void testTensorFloat16Add() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < tensorC.getSize(); i++) {
      assertThat((double) tensorC.get(i).getFloat32(), closeTo(HalfFloat.add(tensorA.get(i), tensorB.get(i)).getFloat32(), 0.01f));
    }
  }
