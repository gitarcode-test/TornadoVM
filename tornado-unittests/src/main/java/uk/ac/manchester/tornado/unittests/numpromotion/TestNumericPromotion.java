 ...
 *
 * <p>In a nutshell, byte operations are performed in bytes, but JVM narrows up the result to int
 * (signed), which leads to wrong results, because the output is not a byte anymore, is an integer.
 *
 * <p>Since this is in the JLS spec, we can't do too much regarding Java semantics, but a possible
 * solution is to expose a type in TornadoVM that we can handle like an intrinsic and force not to
 * sign those operations.
 *
 * <p>Affect arithmetic, logical and shift operations when operands are byte and short data types.
 *
 * <p>How to run? <code>
...
public class TestNumericPromotion extends TornadoTestBase {
  ...
  public void testBitwiseOr() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < result.getSize(); i++) {
      assertThat(sequential.get(i), equalTo(result.get(i)));
    }
  }
  ...
  public void testBitwiseNot() throws TornadoExecutionPlanException {
    ...
    ByteArray input =
        ByteArray.fromElements(
            (byte) 0, (byte) 0, (byte) 127, (byte) -127, (byte) 1, (byte) -1, (byte) 1, (byte) 1);
  ...
  public void testSubtraction() throws TornadoExecutionPlanException {
    ...
    ByteArray input =
        ByteArray.fromElements(
            (byte) 125, (byte) 125, (byte) 125, (byte) 125, (byte) 1, (byte) 1, (byte) 1, (byte) 1);
  ...
  public void testDivision() throws TornadoExecutionPlanException {
    ...
    ByteArray input =
        ByteArray.fromElements(
            (byte) 2, (byte) 2, (byte) 2, (byte) 2, (byte) 1, (byte) 1, (byte) 1, (byte) 1);
