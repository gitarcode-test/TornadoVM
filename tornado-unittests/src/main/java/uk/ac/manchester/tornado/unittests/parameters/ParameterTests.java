...
public class ParameterTests extends TornadoTestBase {
  ...
  /**
   * This method is only for testing the Task-Graph and ExecutionPlan logic when having scalar
   * values as parameters. Note that this method does not return any value is stored in a local
   * variable. Thus, it does not scale the method scope.
   */
  ...
  /**
   * This test throws a {@link TornadoRuntimeException} because scalar values are used as output
   * parameters. This type of code is not legal in TornadoVM.
   */
  @Test
 ...
 assertThrows(TornadoRuntimeException.class, () -> {
  ...
  public void testScalarParameters03() {
    ...
    for (int i = 0; i < z.getSize(); i++) {
      assertThat(y + x.get(i), equalTo(z.get(i)));
    }
  }
}
