...
public class TestLoopTransformations extends TornadoTestBase {
  ...
  private static void matrixVectorMultiplication(
      final FloatArray A, final FloatArray B, final FloatArray C, final int size) {
  ...
  public void testPartialUnrollDefault() throws TornadoExecutionPlanException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("s0") //
            ...
            .task(
                "t0",
                TestLoopTransformations::matrixVectorMultiplication,
                matrixA,
                matrixB,
                matrixC,
                size) //
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        assertThat((double) resultSeq.get(i * size + j), closeTo(matrixC.get(i * size + j), 0.01f));
      }
    }
  }
  ...
  public void testPartialUnrollNvidia32() throws TornadoExecutionPlanException {
    ...
    for (int i = 0;
        i < TornadoRuntimeProvider.getTornadoRuntime().getBackend(0).getNumDevices();
        i++) {
      if (TornadoRuntimeProvider.getTornadoRuntime()
          .getBackend(0)
          .getDevice(i)
          .getPlatformName()
          .toLowerCase()
  ...
  public void testPartialUnrollParallelLoops() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        assertThat((double) matrixB.get(i * N + j), closeTo(resultSeq.get(i * N + j), 0.1));
      }
    }
  }
  // CHECKSTYLE:ON
