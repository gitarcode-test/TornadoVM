...
public class TestMatrixTypes extends TornadoTestBase {
  ...
  private static void testMatricesFloats(final int X, final int Y)
    ...
    for (int i = 0; i < X; i++) {
      for (int j = 0; j < Y; j++) {
        assertThat((double) matrixB.get(i, j), closeTo(matrixA.get(i, j) + matrixA.get(i, j), 0.01f));
      }
    }
  }
  ...
  private static void testMatrixDoubles(final int X, final int Y)
    ...
    for (int i = 0; i < X; i++) {
      for (int j = 0; j < Y; j++) {
        assertThat((double) matrixB.get(i, j), closeTo(matrixA.get(i, j) + matrixA.get(i, j), 0.01));
      }
    }
  }
  ...
  public void testMatrix10() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        for (int k = 0; k < N; k++) {
          assertThat((double) matrixB.get(i, j, k), closeTo(matrixA.get(i, j, k) + matrixA.get(i, j, k), 0.01f));
        }
      }
    }
  }
