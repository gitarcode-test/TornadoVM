...
public class PrebuiltTests extends TornadoTestBase {
  ...
  public void testPrebuilt01() throws TornadoExecutionPlanException {
    ...
    // When using the prebuilt API, we need to define the WorkerGrid, otherwise it will launch 1
    // thread
  ...
  public void testPrebuilt02SPIRV() throws TornadoExecutionPlanException {
    ...
    assertThat(512f, closeTo(finalSum, 0.0f));
  }

  /**
   * This test case verifies that the {@link PrebuiltTests#testPrebuilt03SPIRV} runs correctly
   * though a SPIR-V or OpenCL runtime if the device supports SPIR-V.
   *
   * <p>Expected outcome: - If the current backend type is PTX, the test should have thrown
   * unsupported exception. - The test should succeed if a SPIR-V supported device is available, or
   * should use OPENCL as the backend with a device that device supports SPIR-V; otherwise, the test
   * should fail.
   */
  @Test
  public void testPrebuilt03SPIRV() throws TornadoExecutionPlanException {
    ...
    assertThat(64f, closeTo(finalSum, 0.0f));
  }
  ...
  public void testPrebuilt04SPIRVThroughOpenCLRuntime() throws TornadoExecutionPlanException {
    ...
    if (device == null) {
      assertNotBackend(
          TornadoVMBackendType.OPENCL,
          "No SPIRV supported device found with the current OpenCL backend. The OpenCL version must"
              + " be >= 2.1 to support SPIR-V execution.");
    }
  ...
  /**
   * This test is intended to be passed with multiple backends (e.g., OpenCL and PTX). The PTX
   * backend needs to be installed. Otherwise, an exception is thrown.
   *
   * <p>How to run? <code>
  ...
  public void testPrebuiltMutiBackend() throws TornadoExecutionPlanException {
    ...
    List<TornadoBackend> ptxBackend =
        tornadoDeviceMap.getBackendsWithPredicate(
            backend -> backend.getBackendType() == TornadoVMBackendType.PTX);

    if (ptxBackend == null || ptxBackend.isEmpty()) {
      throw new TornadoVMPTXNotSupported(
          "Test designed to run with multiple backends, including a PTX backend");
    }
