...
/**
 * Test to check functionality of benchmarks available in the compute-benchmark package.
 *
 * <p>How to run? <code>
...
public class ComputeTests extends TornadoTestBase {
  ...
  private static void nBody(int numBodies, FloatArray refPos, FloatArray refVel) {
    for (@Parallel int i = 0; i < numBodies; i++) {
      ...
      for (int k = 0; k < 3; k++) {
        refPos.set(
            body + k,
            refPos.get(body + k) + refPos.get(body + k) * DELTA + 0.5f * acc[k] * DELTA * DELTA);
  ...
  public static void validate(
      int numBodies,
      FloatArray posTornadoVM,
      FloatArray velTornadoVM,
      FloatArray posSequential,
      FloatArray velSequential) {
    for (int i = 0; i < numBodies * 4; i++) {
      assertThat((double) posSequential.get(i), closeTo(posTornadoVM.get(i), 0.1f));
      assertThat((double) velSequential.get(i), closeTo(velTornadoVM.get(i), 0.1f));
    }
  }

  public static void computeDFT(
      FloatArray inreal, FloatArray inimag, FloatArray outreal, FloatArray outimag) {
  ...
  public static void computeDFTFloat(
      FloatArray inreal, FloatArray inimag, FloatArray outreal, FloatArray outimag) {
    int n = inreal.getSize();
    for (@Parallel int k = 0; k < n; k++) { // For each output element
      float sumReal = 0;
  ...
  public static void computeDFTVector(
      VectorFloat4 inreal, VectorFloat4 inimag, VectorFloat4 outreal, VectorFloat4 outimag) {
  ...
  private static float cnd(float X) {
    ...
    float y =
        one
            - oneBySqrt2pi
                * TornadoMath.exp(-X * X / two)
                * t
                * (c1 + t * (c2 + t * (c3 + t * (c4 + t * c5))));
    return (X < zero) ? (one - y) : y;
  }

  private static void blackScholesKernel(
      FloatArray input, FloatArray callResult, FloatArray putResult) {
  ...
  private static void checkBlackScholes(
      FloatArray call, FloatArray put, FloatArray callPrice, FloatArray putPrice) {
    double delta = 1.8;
    for (int i = 0; i < call.getSize(); i++) {
      assertThat((double) call.get(i), closeTo(callPrice.get(i), delta));
      assertThat((double) put.get(i), closeTo(putPrice.get(i), delta));
    }
  }
  ...
  private static void computeMatrixVectorFloat4(
      Matrix2DFloat4 matrix, VectorFloat4 vector, VectorFloat output) {
    for (@Parallel int i = 0; i < matrix.getNumRows(); i++) {
      float sum = 0;
      for (int j = 0; j < matrix.getNumColumns(); j++) {
  ...
  private void validateDFT(
      int size, FloatArray inReal, FloatArray inImag, FloatArray outReal, FloatArray outImag) {
    ...
    for (int i = 0; i < size; i++) {
      assertThat((double) outImagSeq.get(i), closeTo(outImag.get(i), 0.1f));
      assertThat((double) outRealSeq.get(i), closeTo(outReal.get(i), 0.1f));
    }
  }

  private void validateDFTVector(
      int size,
      VectorFloat4 inReal,
      VectorFloat4 inImag,
      VectorFloat4 outReal,
      VectorFloat4 outImag) {
  ...
  public void testDFTVectorTypes() throws TornadoExecutionPlanException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("dft") //
            .transferToDevice(DataTransferMode.FIRST_EXECUTION, inReal, inImag) //
            .task(
                "withVectors", ComputeTests::computeDFTVector, inReal, inImag, outReal, outImag) //
            .transferToHost(DataTransferMode.EVERY_EXECUTION, outReal, outImag);
  ...
  public void testHilbert() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < NROWS; i++) {
      for (int j = 0; j < NCOLS; j++) {
        assertThat((double) seq.get(i * NROWS + j), closeTo(output.get(i * NROWS + j), 0.1f));
      }
    }
  }
  ...
  public void testMontecarlo() throws TornadoExecutionPlanException {
    ...
    assertThat((double) sumSeq, closeTo(sumTornado, 0.1));
  }
  ...
  private void validateMandelbrot(int size, ShortArray output) {
    ...
    for (int i = 0; i < size; i++)
      for (int j = 0; j < size; j++)
        assertThat(result.get(i * size + j), equalTo(output.get(i * size + j)));
  }
  ...
  public void testEuler() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < size; i++) {
      assertThat(outputAT.get(i), equalTo(outputA.get(i)));
      assertThat(outputBT.get(i), equalTo(outputB.get(i)));
      assertThat(outputCT.get(i), equalTo(outputC.get(i)));
      assertThat(outputDT.get(i), equalTo(outputD.get(i)));
      assertThat(outputET.get(i), equalTo(outputE.get(i)));
    }
  }
  ...
  public void testRenderTrack() throws TornadoExecutionPlanException {
    ...
    for (int x = 0; x < n; x++) {
      for (int y = 0; y < m; y++) {
        assertThat((double) outputJava.get(x, y).getX(), closeTo(outputTornadoVM.get(x, y).getX(), 0.1));
        assertThat((double) outputJava.get(x, y).getY(), closeTo(outputTornadoVM.get(x, y).getY(), 0.1));
        assertThat((double) outputJava.get(x, y).getZ(), closeTo(outputTornadoVM.get(x, y).getZ(), 0.1));
      }
    }
  }
  ...
  public void testJuliaSets() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        result.set(
            i * size + j,
            Color.HSBtoRGB(hue.get(i * size + j) % 1, 1, brightness.get(i * size + j)));
      }
    }
    ...
    for (int i = 0; i < hueSeq.getSize(); i++) {
      assertThat((double) hueSeq.get(i), closeTo(hue.get(i), delta));
      assertThat((double) brightnessSeq.get(i), closeTo(brightness.get(i), delta));
    }
  }
  ...
  public void matrixVector() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < vectorFloat.size(); i++) {
      assertThat((double) resultSeq.get(i), closeTo(result.get(i), 0.01f));
    }
  }
  ...
  public void matrixVectorFloat4() throws TornadoExecutionPlanException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("graph") //
            ...
            .task(
                "mv",
                ComputeTests::computeMatrixVectorFloat4,
                matrix2DFloat,
                vectorFloat,
                result) //
            .transferToHost(DataTransferMode.EVERY_EXECUTION, result);
