...
/**
 * Test running two and three tasks in parallel on two devices on the same backend.
 *
 * <p>How to test? <code>
...
public class TestMultipleTasksMultipleDevices extends TornadoTestBase {
  ...
  private static void assertAvailableDevices() {
    if (TornadoRuntimeProvider.getTornadoRuntime().getBackend(0).getNumDevices() < 2) {
      throw new TornadoVMMultiDeviceNotSupported(
          "This test needs at least + " + 2 + " devices enabled");
    }
  }
  ...
  public void testTwoTasksTwoDevicesSharedReadOnlyRead() throws TornadoExecutionPlanException {
    ...
    for (int i = 0; i < a.getSize(); i++) {
      assertThat(a.get(i), equalTo((b.get(i) * i)));
      assertThat(d.get(i), equalTo(12L * c.get(i) + b.get(i)));
    }
  }
}
