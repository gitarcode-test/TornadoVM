...
public class TestVectorAPI extends TornadoTestBase {
   ...
   *
   * @param vector1 The first float array.
   * @param vector2 The second float array.
   * @param species The VectorSpecies defining the SIMD instructions width.
   * @return An array containing the result of the parallel addition.
   */
  private float[] parallelVectorAdd(
      FloatArray vector1, FloatArray vector2, VectorSpecies<Float> species) {
            ...
            i -> {
              ...
              FloatVector vec1 =
                  FloatVector.fromMemorySegment(
                      species, vector1.getSegment(), offsetIndex, ByteOrder.nativeOrder());
              FloatVector vec2 =
                  FloatVector.fromMemorySegment(
                      species, vector2.getSegment(), offsetIndex, ByteOrder.nativeOrder());
  ...
  public void test64BitVectors() {
    ...
    MatcherAssert.assertArrayEquals(result, referenceResult.toHeapArray(), DELTA);
  }
