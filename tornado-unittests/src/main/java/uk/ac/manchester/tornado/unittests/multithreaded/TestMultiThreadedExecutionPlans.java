...
/**
 * Test to check the invocation of multiple execution plans from different Java threads. The
 * different execution plans are built using the same instance of the Task-Graph.
 *
 * <p>How to run?
 *
 * <p><code>
...
public class TestMultiThreadedExecutionPlans extends TornadoTestBase {
  ...
  private static void computeForThread1(
      FloatArray input, FloatArray output, KernelContext context) {
   ...
   *
   * <p>In practice, each execution plan has its own instance of an Immutable Task Graph. However,
   * some internal data structure might be shared. This test checks that TornadoVM can run with
   * multiple execution plans using the same graph of computation.
   *
  ...
  public void test01() throws InterruptedException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("check") //
            ...
            .task(
                "compute01",
                TestMultiThreadedExecutionPlans::computeForThread1,
                input,
                output,
                context) //
  ...
  public void test02() throws InterruptedException {
    ...
    TaskGraph taskGraph =
        new TaskGraph("check") //
            .transferToDevice(DataTransferMode.EVERY_EXECUTION, input) //
            .task(
                "compute01", TestMultiThreadedExecutionPlans::computeForThread2, input, output) //
            .transferToHost(DataTransferMode.EVERY_EXECUTION, output);
